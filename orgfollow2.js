//Follows organizations instead of people
//For casperjs
...
function evalfollow()
{
	this.waitFor(function check() {
		...
		var num = this.evaluate(function() {
			return $("li.member-list-item").length;
		});

		var newarr = this.evaluate(function() {
			var arr = [];
			
			$("li.member-list-item").each(function( index ) {
				
				if ( this.children[1].children[0] === undefined)
				{
					return;
				}
				var isfollow = this.children[1].children[0].className.split(' ').indexOf("on");
				if (isfollow === -1)
				{
					arr.push( this.children[0].children[1].children[0].children[0].innerHTML );
					this.children[1].children[0].children[0].children[0].children[1].click()
				}
			});
...
function innerloop()
{
	...
	casper.echo("https://github.com/orgs/"+target+"/people?page="+pagenum.toString());
	casper.thenOpen(("https://github.com/orgs/"+target+"/people?page="+pagenum.toString()) , evalfollow);	
	return;
}
